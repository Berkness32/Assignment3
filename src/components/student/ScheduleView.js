...
const ScheduleView = (props) => {
  ...
  const fetchEnrollments = async () => {
    try {
      const response = await fetchWithJWTHeader(
        `${SERVER_URL}/enrollments?studentId=${sessionStorage.getItem("userId")}&year=${term.year}&semester=${term.semester}`,
      );
      if (response.ok) {
  ...
  return (
    <div>
      ...
      <button id="getSchedule" type="submit" onClick={fetchEnrollments}>
        Get Schedule
      </button>
      <br />
      <br />
        ...
        <tbody>
          {enrollments.map((s) => (
            <tr key={s.enrollmentId}>
              ...
              <td>{s.times}</td>
              <td>
                <Button onClick={onDelete}>Drop</Button>
              </td>
            </tr>
          ))}
        </tbody>
