...
const AssignmentsView = (props) => {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await fetchWithJWTHeader(
        `${SERVER_URL}/sections/${secNo}/assignments`,
      );
      if (response.ok) {
  ...
  const add = (assignment) => {
    ...
    fetchWithJWTHeader(`${SERVER_URL}/assignments`, {
      method: "POST",
      headers: {
  ...
  const doDelete = (e) => {
    ...
    fetchWithJWTHeader(`${SERVER_URL}/assignments/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
            ...
            <tbody>
              {assignments.map((a) => (
                <tr key={a.id}>
                  ...
                  <td>{a.dueDate}</td>
                  <td>
                    <AssignmentGrade assignment={a} />
                  </td>
                  <td>
                    <AssignmentUpdate assignment={a} save={save} />
                  </td>
                  <td>
