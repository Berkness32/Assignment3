  ...
  const fetchSections = async (year, semester) => {
    ...
    try {
      const response = await fetchWithJWTHeader(
        `${SERVER_URL}/sections?email=dwisneski@csumb.edu&year=${year}&semester=${semester}`,
      );
      if (response.ok) {
            ...
            <tbody>
              {sections.map((s) => (
                <tr key={s.secNo}>
                  ...
                  <td>{s.times}</td>
                  <td>
                    <Link to="/enrollments" id="enrollment" state={s}>
                      Enrollments
                    </Link>
                  </td>
                  <td>
                    <Link to="/assignments" id="assignment" state={s}>
                      Assignments
                    </Link>
                  </td>
                </tr>
