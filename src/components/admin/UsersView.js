...
function UsersView(props) {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetchWithJWTHeader(`${SERVER_URL}/users`);
      if (response.ok) {
  ...
  const deleteUser = async (id) => {
    try {
      const response = await fetchWithJWTHeader(`${SERVER_URL}/users/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user.id}>
              ...
              <td>{user.type}</td>
              <td>
                <AdminUserUpdate user={user} save={saveUser} />
              </td>
              <td>
